...
public class caesarLegions {
    ...
    static long dp[][][][]=new long[105][105][15][15];static int k1,k2;
    ...
    static long count(int n, int m, int a, int b)
    {
        if(dp[n][m][a][b]!=0)return dp[n][m][a][b];
        if(n+m==0)dp[n][m][a][b]=1;
        else {
            long ans=0;
            if (n > 0 && a < k1) ans += count(n - 1, m, a + 1, 0);
            if (m > 0 && b < k2) ans += count(n, m - 1, 0, b + 1);
            dp[n][m][a][b]=ans;
        }
        if(dp[n][m][a][b]>=100000000)dp[n][m][a][b]-=100000000;
        return dp[n][m][a][b];
    }
