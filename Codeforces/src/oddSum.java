...
public class oddSum {
    ...
    static void Blackrise() {                                                //The name Blackrise is my pen name... you can change the name according to your wish
        ...
        long ans=0,posmin=1234567,negmax=-1234567;
        for(int i=0;i<n;i++)
        {
            if(ar[i]==0)continue;
            if(ar[i]>0)ans+=ar[i];
            //pl(ar[i]&1);
            if((ar[i]&1)==1) {
                if (ar[i] > 0) posmin = Math.min(posmin, ar[i]);
                else negmax=Math.max(negmax,ar[i]);
            }
        }
        if((ans%2)==0)
            ans=Math.max(ans-posmin,ans+negmax);
        pl(ans);//pl(posmin);pl(negmax);
