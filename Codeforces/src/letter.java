...
public class letter  {
    ...
    static void Blackrise() {                                                //The name Blackrise is my pen name... you can change the name according to your wish
        ...
        for (int i = 1; i <= s.length(); i++)
            if (s.charAt(i - 1) > 96) dp[i][0] = dp[i - 1][0] + 1;
            else dp[i][0] = dp[i - 1][0];
        for (int i = 1; i <= s.length(); i++)
            if (s.charAt(i - 1) < 96) dp[i][1] = dp[i - 1][1]+1;
            else dp[i][1] = dp[i - 1][1];
        long ans=Integer.MAX_VALUE;
        for(int i=0;i<=s.length();i++)
            ans=Math.min(ans,dp[i][0]+dp[s.length()][1]-dp[i][1]);
        //for(int i=0;i<=s.length();i++)p("("+dp[i][0]+" "+dp[i][1]+") ");
        pl(ans);
