...
public class treasureHunt {
    ...
    static void Blackrise() {                                                //The name Blackrise is my pen name... you can change the name according to your wish
        ...
        long la,lb,lc=0;la=lb=lc;
        int ar[]=new int[125];
        for(int i=0;i<sb.length();i++)la=Math.max(la,++ar[sb.charAt(i)]);

        la=(la+n)%(sb.length()+1);
        ar=new int[125];
        sb=new StringBuilder(ns());
        for(int i=0;i<sb.length();i++)lb=Math.max(lb,++ar[sb.charAt(i)]);
        lb=(lb+n)%(sb.length()+1);
        ar=new int[125];
        sb=new StringBuilder(ns());
        for(int i=0;i<sb.length();i++)lc=Math.max(lc,++ar[sb.charAt(i)]);
        lc=(lc+n)%(sb.length()+1);
        if(la>lb && la>lc)pl("Kuro");
        else if(lb>la && lb>lc)pl("Shiro");
        else if(lc>la && lc>lb)pl("Katie");
        else pl("Draw");
