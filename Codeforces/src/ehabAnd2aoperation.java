...
public class ehabAnd2aoperation {
    ...
    static void Blackrise() {                                                //The name Blackrise is my pen name... you can change the name according to your wish
        ...
        Arrays.sort(ar);
        ArrayList<Long> art=new ArrayList<>();art.add(ar[0]);
        for(int i=1;i<n;i++)if(ar[i]!=ar[i-1])art.add(ar[i]);
        long temp=0;
        for(int i=0;i<k;i++)
        {
            if(i<art.size()){pl(art.get(i)-temp);temp=art.get(i);}
            else pl(0);
