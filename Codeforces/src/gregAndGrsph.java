...
public class gregAndGrsph {
    ...
    static void Bolo_Jai_Mata_Di() {
        ...
        int ar[]=new int[n+1];
        for(int i=0;i<n;i++)ar[i]=ni()-1;
        long ans[]=new long[n];

        for(int k=n-1;k>=0;k--)
        {
            int x=ar[k];vis[x]=1;
            floydwarshal(adj,vis,x);
            ans[k]=cal(adj,vis);
        }
        for(long i:ans)p(i);
    ...
    static void floydwarshal(int floydw[][],int vis[],int x) {
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                floydw[i][j] = min(floydw[i][j], floydw[i][x] + floydw[x][j]);
            }
        }
    }

    static long cal(int floydw[][],int vis[])
    {
        long ans=0;
        for(int i=0;i<n;i++)
        {
            for(int j=0;j<n;j++)
            {
                if(vis[i]+vis[j]==2)ans+=floydw[i][j];
            }
        }
        return ans;
    }
