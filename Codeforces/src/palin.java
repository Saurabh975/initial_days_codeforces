...
public class palin {
    ...
    static void Bolo_Jai_Mata_Di() {
        ...
        while(t-->0)
        {
            ...
            boolean sara9=true;n=ch.length;
            for(int i=0;i<n;i++) {
                if (ch[i] != '9') {
                    sara9 = false;
                    break;
            ...
            if(sara9) {
                pws(1);
                for (int i = 1; i < n; i++) pws(0);
                pl(1);
            }
            else
            {
                boolean pehle_ka_kuch_add_kar_pynge=false;
                for(int i=n/2-1;i>=0;i--)
                {
                    if(ch[i]>ch[n-i-1])
                    {
                        pehle_ka_kuch_add_kar_pynge=true;break;
                    }
                    else if(ch[i]<ch[n-i-1])break;
                }
                if(pehle_ka_kuch_add_kar_pynge)
                {
                    for(int i=n/2-1;i>=0;i--)
                        ch[n-i-1]=ch[i];
                }
                else
                {
                    ...
                    for(i=n/2-1;i>=0;i--)ch[n-i-1]=ch[i];
                }
                for(char c:ch)pws(c);pl();
            }
        }
