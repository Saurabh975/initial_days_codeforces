...
public class prefixesAndsuffixes {
    ...
    static void Blackrise() {                                                //The name Blackrise is my pen name... you can change the name according to your wish
        ...
        for (int i = 0; i < 2 * n - 2; i++) {
            ar.add(new StringBuilder(ns()));
            freq[ar.get(i).charAt(0) - 97]++;
            if (ar.get(i).length() == n - 1) {
                if (s1.length() == 0) s1 = ar.get(i);
                else s2 = ar.get(i);
            }
        }
        char ch = 'a';
        ...
        String c = a.concat(Character.toString(b.charAt(n - 2)));
        String d = b.concat(Character.toString(a.charAt(n - 2)));

        if (c.substring(0, n - 1).equals(s1.toString()) && c.substring(1, n).equals(s2.toString()) && c.charAt(0) == ch)
            s1 = new StringBuilder(c);
        else s1 = new StringBuilder(d);

        //pl(s1);

        Map<String, Integer> map = new HashMap<>();
        for (int i = 0; i < 2 * n - 2; i++) {
            if (ar.get(i).toString().equals(s1.substring(0, ar.get(i).length())) && !map.containsKey(ar.get(i).toString())) {
                pws("P");
                map.put(ar.get(i).toString(), 1);
            } else pws("S");
        }
        pl();
