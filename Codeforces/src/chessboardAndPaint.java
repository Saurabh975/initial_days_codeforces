...
public class chessboardAndPaint {
    ...
    static void Blackrise() {                                                //The name Blackrise is my pen name... you can change the name according to your wish
        ...
        while(t-->0)
        {
            long n=ni();
            long m=ni();
            long x1=ni(),y1=ni(),x2=ni(),y2=ni(),x3=ni(),y3=ni(),x4=ni(),y4=ni();

            long black=black(x1,y1,x2,y2);
            long white=white(x3,y3,x4,y4);

            long ans= (n*m+1)/2+black-white;

            long minx=Math.min(x2,x4),maxx=Math.max(x1,x3);
            long miny=Math.min(y2,y4),maxy=Math.max(y1,y3);

            if(minx>=maxx&&miny>=maxy)
                ans-=black(maxx,maxy,minx,miny);
            pl(ans+" "+(n*m-ans));
        }
    ...
    static long  white(long x1, long y1, long x2, long y2)
    {
        long n=x2-x1+1;
        long m=y2-y1+1;
//        if((x1+y1)%2==0)return (n*m+1)/2;
//        else return (n*m)/2;
        return (n*m+((x1+y1)%2==0?1:0))/2;
    }
