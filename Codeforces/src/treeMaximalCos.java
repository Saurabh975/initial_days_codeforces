...
public class treeMaximalCos {

    static long cost[]=new long[200005];
    static long ar[]=new long[200005];
    static long costans[]=new long[200005];
    static ArrayList<Integer> adj[];
    static long sum=0,temp=0;
    ...
    static void dfs1(int cur, int prev, int level)
    {
        temp+=level*ar[cur];
        for(int u:adj[cur])
        {
    ...
    static void dfs2(int cur, int prev)
    {
        for(int u:adj[cur])
        {
            if(u!=prev)
            {
                costans[u]=costans[cur]+sum-(cost[u]*2);
                dfs2(u,cur);
            }
        }
    }
