    ...
    static void Blackrise() {                                                //The name Blackrise is my pen name... you can change the name according to your wish
        ...
        while (t-- > 0) {
            ...
            for (int i = 0; i < sb.length() / 2; i++) {
                if (sb.charAt(i) != sb.charAt(sb.length() - 1 - i)) flag = 1;
                if (i != 0 && sb.charAt(i) != sb.charAt(i - 1)) flag1 = 1;
            }
            if (sb.length() % 2 == 1)
                if (sb.charAt(sb.length() / 2) != sb.charAt(sb.length() / 2 + 1)) flag1 = 1;
            if (flag == 1) pl(sb);
            else if (flag1 == 0) pl(-1);
            else pl(sb.substring(1)+sb.substring(0,1));
