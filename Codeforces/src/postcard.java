...
public class postcard {
    ...
    static void Bolo_Jai_Mata_Di() {
        ...
        int barf=0,dandi=0,shabd=0;
        for(int i=0;i<sb.length();i++)
        {
            if(sb.charAt(i)=='*')barf++;
            else if(sb.charAt(i)=='?')dandi++;
            else shabd++;
        }
        int sabseChotaword=shabd-barf-dandi;
        int sabseBadaWord=barf==0?shabd:123456789;

        if(n<sabseChotaword || n>sabseBadaWord)
        {
        ...
        for(int i=0;i<sb.length();i++)
        {
            ...
            {
                if(shabd>n){
                    s.deleteCharAt(s.length()-1);shabd--;
                }
            }
            else
            {
