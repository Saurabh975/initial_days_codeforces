...
public class multiplicationTable {
    ...
    static void Bolo_Jai_Mata_Di() {
        ...
        long l=0,r=(n*1L)*(m*1L);//pl(r);
        while(l<r)
        {
            long mid=(l+r)>>1;//pl(getThePosition(mid)+" "+(getThePosition(mid)>=k));
            if(getThePosition(mid)>=k)r=mid;
            else l=mid+1;
        }
        pl(l);
    ...
    static long getThePosition(long x)
    {
        long c=0;
        for(int i=1;i<=n;i++)
            c+=(long)min(m,x/(long)i);
        return c;
    }
